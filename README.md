...
> 需要 **Minecraft 1.21.1 / NeoForge** 版本？切到 **`1.21.1-neoforge` 分支**（功能一致，工程直接放在该分支的仓库根目录）。
...
想自己编译：把 6 个上游 jar 放进 `libs/`（清单见 [`libs/README.md`](libs/README.md)），然后 `./gradlew build`，产物在 `build/libs/`。
...
* **电路不算"输入物品"**：电路在 GT 配方里也是 `chance == 0`，但它既不是模具也不是材料 —— 它只是写在样板里、告诉机器该用哪个电路的一条**指令**。所以现在直接把电路从样板的输入里摘掉：AE 既不会把它算进需求量（**"使用/消耗物品"列表里完全不出现**），也不会去网络或 CPU 里取它、更不会推进机器。机器那边由本模组直接写电路槽，库存为 0 也能开工。注意读"样板里写的是哪个电路"时必须读原始样板（包装后的输入已经没有它了）；
